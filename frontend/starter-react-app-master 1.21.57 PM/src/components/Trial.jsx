...
const DoctorViewAppointments = (props) => {
    ...
    useEffect(() => {
        DoctorService.getDoctorAppintmentsbyID(localStorage.getItem('Did')).then((res) => {
            setPatientAppts(res.data);
            console.log(res.data);
            //console.log("lll",res.data[2][1]);
            for(var i=0;i<res.data.length;i++)
            {
                const axios = require('axios').default;
                axios.post('http://127.0.0.1:9090/patient/getpatientprofiledetails/'+res.data[i][1], {
            
              })
              .then((res1) => {
                if(res1.data){ 
                    pnames.push(res1.data.name);
                  console.log("result",pnames);
                }
                else{
                    console.log("NOO!!");
            ...
            console.log("type",typeof(localStorage.getItem('Did')));
