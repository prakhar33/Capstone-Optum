...
const HospitalManageDoctor = (props) => {
    ...
    const handleDeleteAppt = (doctor_name) => {
        swal({
            title: "Are you sure?",
          ...
          .then((willDelete) => {
            if (willDelete) {
                HospitalService.deleteDoctor(doctor_name)
            .then((res)=> {
                swal({
                    icon: "success",
            ...
            // PatientService.getAllPatientAppts(localStorage.getItem('Pid'))
            // .then((res1) => {
    ...
    useEffect(() => {
        HospitalService.viewDocByHsptl(localStorage.getItem('Hid')).then((res) => {
    ...
    console.log("kkkkk",patientAppts);
...
{localStorage.getItem("Hlogin")==='1' ?
        <div>
            ...
            {setSlots && showSlots()}
            <center style={{marginTop:"10px"}}><h2>Manage Doctors</h2></center>
                ...
                <tbody>
                    {
                        patientAppts?.map(
                            patientAppt=>
                            <tr key={patientAppt}>
                                <td >{patientAppt.doctor_id}</td>
                                <td >{patientAppt.name}</td>
                                <td>{SlotToTime(patientAppt.start_time).start}</td>
                                <td>{SlotToTime(patientAppt.end_time-1).end}</td>
                                <td >{patientAppt.mobileNumber}</td>
                                <td >{patientAppt.specialization}</td>
                                ...
                                <td><button className='deleteButton' onClick={()=>{handleDeleteAppt(patientAppt.name)}} title="Delete"><span class="fa fa-trash"></span></button></td>
                            </tr>
                        )
                    }
                </tbody>
...
export default HospitalManageDoctor;