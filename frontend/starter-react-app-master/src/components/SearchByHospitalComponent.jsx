...
const SearchByHospitalComponent=()=>{
    ...
    //   const [fetcheddoctors , setfetcheddoctors] = useState([]);
      ...
      useEffect(()=>{
        ...
        //alert("Hi");
        axios.get('http://localhost:9090/doctor/getAll')
      .then((res) => {
          console.log("Received doctor list to get hospital names from it");
...
const navigate=useNavigate();
    ...
    const handleSubmit=(e)=> {
        const axios = require('axios').default;
        console.log("hh",selectedhospital);
        axios.post(
                ...
                <option value="" selected disabled>Select Hospital</option>
                    {
                        fetchedhospitals.map((hospital) => {
                            ...
                            return <option value={hospital.hospital_name}>{hospital.hospitalName}</option>;
                        })
                    }
                </select>
        </div>
